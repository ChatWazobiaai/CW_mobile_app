...
import {BoldText, RegularText} from '../Texts/CustomTexts/BaseTexts'; // Update the path as necessary
...
const CustomButton: React.FC<CustomButtonProps> = ({
  ...
  const backgroundColor =
    loading || disabled ? Colors.grayColor65 : Colors.whiteColor;
  const finalTextColor = textColor || Colors.blackColor; // Use passed textColor or default to white
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <BoldText fontSize={19} color={finalTextColor}>
          {title}
        </BoldText>
      )}
    </Pressable>
...
const styles = StyleSheet.create({
  button: {
    borderRadius: 12,
    paddingVertical: 20, // Adjust for vertical padding
